...
void CrashPutChar(void *ptr __unused, char data) {
  struct CrashLog *cl = &CrashLog;
 ...
 *
 * I want crash site location to be printed out. This includes analysis
 * which (if any) loadable hunk the offending instruction belongs to.
 *
 * TODO Could we make interrupt stack to be placed safely?
 *
 * Once we're here, we know that things went terribly bad and we can assume
 * very little about the state of the system. Let's list these assumptions.
 *
 * 1) This code, when loaded by bootloader, in all cases will end up somewhere
 * at the end of memory, preferably FAST if available.
 */
